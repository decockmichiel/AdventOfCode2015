...
Distances readDistances(const Strings& lines)
{
    ...
    Distances distances(cities.size(), std::vector<int>(cities.size(), INT_MAX));
...
int findShortestRoute(int                     minDistance,
                      ...
                      const Distances&        distances)
{
    ...
    for (int city = 0; city != numberOfCities; ++city)
    {
        ...
        if (visitedCities.size() == numberOfCities - 1)
            minDistance = std::min(minDistance, getDistance(distances, cities));
        else

            minDistance = std::min(minDistance, findShortestRoute(minDistance, cities, distances));
    }
...
int main()
{
    ...
    int minDistance = findShortestRoute(INT_MAX, {}, distances);
